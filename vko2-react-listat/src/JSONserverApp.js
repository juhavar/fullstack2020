...
import { useState, useReducer, initialState } from 'react'
...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case "IKA_MUUTTUI":
      ...
      case "LAPSEN_NIMI_MUUTTUI":
        syväKopio[action.data.ihminenIndex].jälkikasvu.ikä = action.data.newText;
        return syväKopio
    case "POISTA_HENKILO":
...
function App() {
  ...
  return (<div>
    ...
    {state.map((item, index) => <div key={item.uid}>
      ...
      <button onClick={() => dispatch({type: "POISTA_HENKILO", data: index })} >Poista henkilö</button>
      {item.jälkikasvu ? <LapsiLista
        dispatch={dispatch} parentIndex={index} lapsiLista={item.jälkikasvu}></LapsiLista> : ""}
    </div>)}


    <button  onClick={() => dispatch({type:"LISAA_HENKILO", })} >Lisää henkilö</button>
  </div>
  );
}
